...
/*   Updated: 2017/01/19 22:39:42 by tbouder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	for (piece_y < len(pieces)) {
		...
		for (piece_x < len(pieces[piece_y])) {
			...
			if (map_x + piece_x > len(board[0]) - 1) {return board, false}
				...
				// ft_print_arr(new_board)
				// fmt.Print("[", piece_y, piece_x, "] on map[", map_y + piece_y, map_x + piece_x, "] (",new_board[map_y + piece_y][map_x + piece_x], ") : ")
				// fmt.Println(string(pieces[piece_y][piece_x]))
	...
	for (map_y < len(board)) {
		...
		for (map_x < len(board[map_y])) {
			...
			if (index < nb_pieces) {
				...
				// if (verif == false) {fmt.Println("FALSE")}
				if (verif == true) {
